...
# ----------- Dataset generator -----------
def generate_dataset(num_orders=100, mode="neat", label_problems=False):
    data = []
    ...
    for i in range(num_orders):
        ...
        if mode in ["delayed", "super_stress"] and random.random() < 0.3:
            step_duration = timedelta(minutes=random.randint(15, 25))
            if mode == "super_stress":
                problem_type.append("Delayed")
        ...
        for idx, step in enumerate(planned_steps):
            ...
            final_yield = 24
            scrap_qty = 0
            if mode in ["quantity", "super_stress"] and random.random() < 0.3:
                final_yield = random.randint(15, 23)
                scrap_qty = 24 - final_yield
                if mode == "super_stress":
                    problem_type.append("Quantity Issue")
            ...
            if label_problems:
                row["Problem_Type"] = ", ".join(problem_type) if problem_type else "None"
            data.append(row)
...
if __name__ == "__main__":
    modes = [
        ...
        ("super_stress", "dataset_super_stress_07.08.csv"),
    ]
    for mode, filename in modes:
        if mode == "super_stress":
            df = generate_dataset(100, mode=mode, label_problems=True)
        else:
